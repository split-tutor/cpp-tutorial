...
C++ 프로그램을 작성하다 보면 다음과 같은 상황이 자주 발생합니다.
...
워낙 자주 나타나는 패턴이기 때문에 변수 이름을 두번 적는 것도 낭비라고 생각해서 인지,
다음과 같이 축약해서 쓰는 경우가 많습니다.
...
위와 같은 문장을 보면,
변수 `x`의 값을 읽어와서 3을 더한 후에 다시 `x`에 덮어쓰는 코드라고 이해하시면 됩니다.
`+` 대신에 `-`, `*`, `/`, `%` 등 연산도 쓰일 수 있습니다.
다만 동작하는 방식은 똑같으니, 다음과 같은 코드가 등장해도 당황하지 않고 읽을 수 있겠죠?

그리고 더 나아가서 `int`, `long` 등 정수 타입 변수 앞 또는 뒤에 `++` 또는 `--`가 붙어있는 경우가 있습니다.

이는 `+= 1` 이나 `-= 1` 마저 줄여서 쓰는 표현입니다.
`x++`또는 `++x`는 변수 `x`의 값을 읽어와서 1을 더한 후에 다시 `x`에 덮어쓰는코드라고 이해하시면 됩니다.
`x--`나 `--x`은 반대의 의미입니다.
`++`나 `--`가 앞에 등장하느냐 뒤에 등장하느냐에 따라서 계산 순서가 조금 달라집니다.
이런 연산자들이 앞에 등장할 때에는 이 연산자가 등장하는 문장을 계산하기 전에, 이 연산자를 계산해주고, 뒤에 등장할 때에는 문장을 계산한 후에 이 연산자를 계산해줍니다. 예를 들면:

```
int x = 3;
int y = 4;
int z = (++x) * (y--) // z: 16,  x: 4, y: 3
```

위 코드가 세 번째 줄을 실행할 때, `++x`가 가장 먼저 실행 돼 `x` 값을 4로 덮어쓰고,
`int z = 4 * 4`를 계산한 후에 `y`에 3을 덮어써 줌으로써 최종적으로는 위와 같은 상태를 갖게 되는 것입니다.
...
특별한 형태의 `for` 반복문을 통해 `std::string`의 각 문자에 대해 연산을 수행하는 코드를 작성할 수 있습니다.
...
바로 `:`를 이용한 range-based `for` 반복문입니다.
나중에 `std::vector`에 대해 배울 때 한 번 더 만나게 될 문법입니다.
